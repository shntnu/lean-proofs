...
theorem Sequence.IsCauchy.convergent {a:Sequence} (h:a.Convergent) : a.IsCauchy := by
  ...
  obtain ⟨N, hN, hclose⟩ := hL (ε / 2) hε2
  refine ⟨N, hN, ?_⟩
  intro n hn m hm
  simp only [Real.Close]
  have hn_am : n ≥ a.m := by simp at hn; omega
  have hm_am : m ≥ a.m := by simp at hm; omega
  have hn' : n ≥ N := by simp at hn; omega
  have hm' : m ≥ N := by simp at hm; omega
  have hn_max : n ≥ max a.m N := by omega
  have hm_max : m ≥ max a.m N := by omega
  simp [hn_max, hm_max]
  have h1 := hclose n (by simp; omega)
  have h2 := hclose m (by simp; omega)
  simp [hn_max, hm_max] at h1 h2
  calc dist (a n) (a m)
      ≤ dist (a n) L + dist L (a m) := dist_triangle _ _ _
    _ = dist (a n) L + dist (a m) L := by rw [dist_comm L (a m)]
    _ ≤ ε / 2 + ε / 2 := by gcongr
    _ = ε := by ring
